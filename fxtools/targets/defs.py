...
def scan_files():
    print(f"Deep scanning {printer.F_GREEN(os.path.abspath(ROOT_DIR))} ... {printer.F_GREY('(Please wait)')}")
...
def fetch_and_print_docs(query, info):
    ...
    print(f"{printer.F_GREEN('[Documentation]')} {printer.F_GREY(os.path.basename(resolved_source))}")

    if docstring:
        # Indent docstring text cleanly for terminal alignment
        print("\n".join(f" | {line}" for line in docstring.splitlines()))
    else:
        print(f"  {printer.F_GREY('No docstring found')}")
...
def run():
    ...
    while True:
        try:
            query = input(f"\x1b[1m{printer.F_BLUE('>>> ')}\x1b[0m").strip()
            ...
            if "*" in query:
                print(f"\n{printer.F_GREY(f'Patterns matching {query!r}')}")
                ...
                if matches:
                    ...
                    printer.error("No matches found.")
                print()
                continue
            ...
            if query in data:
                ...
                print(f"{printer.F_RED(f'{query!r} not found.')}\n")
                if suggestions:
                    suggestions = [s for s in suggestions if s != query]
                    printer.warn("Did you mean:")
...
def help():
    return {
        "USAGE": "fx defs",
        "DESCRIPTION": "Launches an interactive kernel workspace indexer and lookup terminal interface\n"
                       "to map out declarations, source definitions, usages, and internal documentation blocks.\n"
                       "You can input the name of a function, macro, or type to see its definition and usages."
    }
