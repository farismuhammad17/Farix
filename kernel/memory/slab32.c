...
Slab32* create_slab32(uint16_t object_size) {
    ...
    if (unlikely(!phys)) {
        t_print("create_slab32: pmm_alloc_page failed");
        uart_print("create_slab32: pmm_alloc_page failed\n");
        return NULL;
    }
...
void* slab_alloc32(Slab32* head) {
    if (unlikely(!head || head->magic != SLAB32_MAGIC)) {
        t_print("slab_alloc32: Invalid head");
        uart_print("slab_alloc32: Invalid head\n");
        return NULL;
    }
    ...
    if (unlikely((addr & 0xFFFFF000) != ((uintptr_t) curr & 0xFFFFF000))) {
        t_printf("slab_alloc32: Page boundary overflow");
        uart_printf("slab_alloc32: Page boundary overflow");
        return NULL;
    }
